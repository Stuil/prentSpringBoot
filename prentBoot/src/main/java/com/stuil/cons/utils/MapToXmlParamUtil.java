...
public class MapToXmlParamUtil {
    ...
    public static String createLinkStringByGet(Map<String, Object> params) {
        ...
        for (int i = 0; i < keys.size(); i++) {
            ...
            // 签名原始串中，字段名和字段值都采用原始值，不进行URLEncode。
           // value = URLEncoder.encode(value, "UTF-8");
            //拼接时，不包括最后一个&字符,且为空字段不参加签名
    ...
    public static Map<String, Object> getUrlParams(String param) {
        ...
        map.put("a","9991");
        map.put("c","199");
        String s1= (String) map.get("a");
        map.put("a","");
        System.out.println(s1);
   //  map=   EntityUtils.entityToMap(initReq);
        System.out.println(createLinkStringByGet(map));
        System.out.println(getUrlParams(createLinkStringByGet(map)));
        System.out.println(multilayerMapToXml(map,true));
        System.out.println(multilayerXmlToMap(multilayerMapToXml(map,true)).get("xml"));
    }
}
